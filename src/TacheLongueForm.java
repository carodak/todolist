...
public class TacheLongueForm extends JFrame implements ActionListener {
  ...
  private JLabel label4 = new JLabel("Date début (jj/MM/aaaa)");
  ...
  public TacheLongueForm(){      

    this.setTitle("Tache Longue");
    ...
    jtf4.addKeyListener(new ClavierListener());
	...
	public void actionPerformed(ActionEvent evt) {
		...
		if (source == b){ //Si l'on a cliqué sur le bouton "ok"
			...
			if (!categorie.isEmpty()){
				...
				A = new TacheLongue (titre,categorie,dateDebut,echeance); //date au format jj/mm/aaaa  
				...
				cat.ajoutCategorie(A.getCat());
				 ...
				 A = new TacheLongue (titre,"sans categorie",dateDebut,echeance); //date au format jj/mm/aaaa  
			
			}
	
			if (!dateDebut.isEmpty() && (A.setEcheanceSring(echeance)==-1 || A.setDateDebutSring(dateDebut)==-1)){
				JOptionPane.showMessageDialog(this, "Une de vos dates n'est pas bonne(date_debut >= jour creation tache > echeance) ","avertissement", JOptionPane.WARNING_MESSAGE);
			}
			
			else if (dateDebut.isEmpty() && A.setEcheanceSring(echeance)==-1){
				JOptionPane.showMessageDialog(this, "Une de vos dates n'est pas bonne(date_debut >= jour creation tache > echeance) ","avertissement", JOptionPane.WARNING_MESSAGE);
			}
